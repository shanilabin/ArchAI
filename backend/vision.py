...
חשוב: תוכניות אדריכליות מכילות לרוב כיתובים, מידות ושטחים מודפסים (למשל "חדר שינה 9.0 מ"ר", מידות בס"מ/מ', סימוני כיריים/כיור/חלון). קרא אותם והשווה לכללים. אם אין מידות — הערך באופן איכותי וסמן רק כשיש חשד ברור.
...
לכל חשד החזר נקודה על התמונה:
- x = אחוז מהקצה השמאלי (0 = שמאל, 100 = ימין)
- y = אחוז מהקצה העליון (0 = למעלה, 100 = למטה)
מקם את הנקודה במרכז האזור הרלוונטי. הערכים בין 0 ל-100 בלבד.
...
החזר אך ורק JSON: {"issues": [{"x": <0-100>, "y": <0-100>, "severity": "warning"/"review"/"info", "description": "<משפט קצר בעברית>"}]}
"""
